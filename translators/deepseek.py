...
class DeepSeekTranslator(TranslatorBase):
    ...
    def prepare_prompt_for_item(self, item: TextItem, source_lang: str, target_lang: str) -> tuple[str, list[str]]:
    ...
    def _store_success(self, item: TextItem, translated: str, source_lang: str, target_lang: str):
        if (
    ...
    def _propagate_by_cache_key(
        ...
        cache = get_cache()
        trans_map = {
            p.cache_key: p.item.translated
            for p in pending
            if self._should_store_translation(p.item, p.item.translated, source_lang)
        }
        if not trans_map:
            return
        for item in items:
